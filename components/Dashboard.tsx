    ...
    <Card title="مهامي النشطة">
      <ul className="space-y-3">
        {myTasks.length > 0 ? myTasks.slice(0, 4).map(task => {
          ...
          return (
            <li key={task.id} className="flex justify-between items-center p-2 rounded-md hover:bg-gray-50">
              <div>
                <p className="font-semibold text-medium-text">{task.title}</p>
                <p className="text-sm text-light-text">{project?.name}</p>
              </div>
              <StatusBadge status={task.status} type="Task" />
            </li>
          );
        }) : <p className="text-light-text">لا توجد مهام مسندة.</p>}
      </ul>
    </Card>
  );
};
...
const KpiCard = ({ title, value, icon: Icon, iconBgColor }: { title: string, value: string | number, icon: React.FC<{className?: string}>, iconBgColor: string }) => (
    <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-100 flex items-center">
        ...
        <Card title="ميزانيات المشاريع">
            <ResponsiveContainer width="100%" height={300}>
                <BarChart data={budgetData}>
                    ...
                    <Tooltip formatter={(value) => new Intl.NumberFormat('ar-BH', { style: 'currency', currency: 'BHD' }).format(Number(value))} />
                    <Legend />
...
const PendingExpensesWidget = ({ expenses }: { expenses: Expense[] }) => (
    <Card title="طلبات المصاريف المعلقة">
        ...
        {expenses.filter(e => e.status === 'Pending').map(expense => (
            <div key={expense.id} className="flex justify-between items-center py-2 border-b">
                ...
                <p className="font-semibold">{new Intl.NumberFormat('ar-BH', { style: 'currency', currency: 'BHD' }).format(expense.amount)}</p>
                <div className="flex gap-2">
                    <button className="text-xs bg-green-500 text-white px-2 py-1 rounded hover:bg-green-600">موافقة</button>
                    <button className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600">رفض</button>
                </div>
            </div>
        ))}
    </Card>
        ...
        {materials.map(material => (
            <div key={material.id} className="mb-4">
                <div className="flex justify-between items-center mb-1">
                    ...
                    <p className={`text-sm ${material.quantity <= material.threshold ? 'text-red-500 font-bold' : 'text-dark-text'}`}>
        ...
        <div className="overflow-x-auto max-h-96">
            <table className="min-w-full text-sm text-right">
                <thead className="bg-gray-50 sticky top-0">
                    <tr>
                        <th className="p-3 text-right font-semibold text-medium-text">المادة</th>
                        <th className="p-3 text-right font-semibold text-medium-text">الكمية</th>
        ...
        <div className="h-full cursor-pointer group" onClick={() => onNavigate && onNavigate('Tasks')}>
            <Card title="المهام النشطة حالياً" className="h-full transition-all group-hover:border-primary group-hover:shadow-xl">
                ...
                {activeTasks.length > 0 ? activeTasks.map(task => {
                    ...
                    return (
                    <div key={task.id} className="p-3 rounded-lg bg-gray-50 border border-gray-200 hover:shadow-md hover:border-primary transition-all">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-light-text hidden sm:block">{user?.name}</span>
                        </div>
                        </div>
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="mt-2 text-medium-text">لا توجد مهام نشطة حالياً. عمل رائع!</p>
                    </div>
                )}
                </div>
            </Card>
...
const ProjectProgressWidget = ({ projects }: { projects: Project[] }) => {
    ...
    if (!clientProject) return <Card title="تقدم المشروع"><p>لا توجد مشاريع لعرضها.</p></Card>;
    ...
    return (
        <Card title={`تقدم المشروع: ${clientProject.name}`}>
            <div className="space-y-4">
                <div>
                    <div className="flex justify-between items-center mb-1">
                        <p className="font-semibold text-lg">نسبة الإنجاز الكلية</p>
            ...
            <table className="min-w-full text-sm text-right">
                <thead className="bg-gray-50">
                    <tr>
                        ...
                        <th className="p-3 text-right font-semibold">تاريخ التسليم</th>
...
const MyEquipmentWidget = ({ currentUser, equipment }: { currentUser: User, equipment: Equipment[] }) => {
    ...
    return (
        <Card title="معداتي">
            {myEquipment ? (
                <div>
                    <h3 className="text-xl font-bold">{myEquipment.name}</h3>
        ...
        <div className="h-full cursor-pointer group" onClick={() => onNavigate && onNavigate('Manpower')}>
            <Card title="ملخص القوى العاملة" className="h-full transition-all group-hover:border-primary group-hover:shadow-xl">
                <div className="flex justify-between items-center mb-4">
                    <p className="text-medium-text">إجمالي العمال</p>
                    <p className="text-3xl font-bold text-dark-text">{totalWorkers}</p>
                </div>
                <div>
                    <h4 className="text-sm font-semibold text-medium-text mb-2">أبرز المهن</h4>
                    <ResponsiveContainer width="100%" height={100}>
                        <BarChart data={topProfessions} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                             ...
                             <Tooltip cursor={{fill: '#F8FAFC'}} formatter={(value) => `${value} عامل`} />
...
const Dashboard: React.FC<DashboardProps> = (props) => {
  ...
  const WelcomeHeader = () => (
    <div className="mb-8">
      ...
      <p className="text-light-text mt-1">إليك نظرة عامة على لوحة التحكم الخاصة بك لليوم.</p>
    </div>
  );
    ...
    <div>
      ...
      <div className="flex justify-end mb-4">
        <button 
          ...
          className="flex items-center gap-2 bg-white border border-gray-300 text-dark-text px-4 py-2 rounded-lg hover:bg-gray-100 transition-colors text-sm font-semibold"
        >
            ...
            <div className="lg:col-span-2">
                <Card>
                    <div className="text-center py-12">
                        <h3 className="text-xl font-semibold text-dark-text">لوحة التحكم فارغة</h3>
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-lg w-full">
                <h3 className="text-xl font-bold text-dark-text mb-6">تخصيص لوحة التحكم</h3>
                <div className="space-y-4 max-h-80 overflow-y-auto">
                    {availableWidgets.map(([key, config]) => (
                        <label key={key} htmlFor={key} className="flex items-center p-3 rounded-md hover:bg-gray-100 cursor-pointer">
                            <input 
                                type="checkbox"
                                id={key}
                            ...
                            />
                            <span className="mr-3 font-medium text-medium-text">{config.name}</span>
                        </label>
                    ))}
                </div>
